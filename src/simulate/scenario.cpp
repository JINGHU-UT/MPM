...
struct RayleighScenar : public Scenario {
	Scalar particle_density( const Vec3 &x ) const override {
...
struct BridsonScenar : public Scenario {
	...
	virtual void init( const Params& ) override {
		center = Vec3( .75*m_config->box[0], .75*m_config->box[1], .5*m_config->box[2] ) ;
...
struct TowerScenar : public Scenario {
	...
	virtual void init( const Params& params ) override {
		center = Vec3( .5*m_config->box[0], .5*m_config->box[1], .5*m_config->box[2] ) ;
		radius = .125 * m_config->box[0] ;
	...
	void add_rigid_bodies( std::vector< RigidBody >& rbs ) const override
	{
		...
		Vec3 pos = center - h0*Vec3(1,1,0)/M_SQRT2 ;
		pos[2] -= .125 * m_config->box[2];
		ls->scale(.5*.125*m_config->box[0]).set_origin( pos ) ;

		rbs.emplace_back( ls, volMass );
		rbs.back().set_velocity( Vec3(hvel/M_SQRT2, hvel/M_SQRT2, zvel), Vec3(avel,0,0) ) ;
	}

	void update( Simulate& simu, Scalar /*time*/, Scalar dt ) const override
	{
...
struct RbPlaneTestScenar : public Scenario {
	...
	void add_rigid_bodies( std::vector< RigidBody >& rbs ) const override
	{
		...
		ls->set_origin( .5 * m_config->box - Vec3(0,0,.25*m_config->box[2]) ) ;
		ls->set_rotation( Vec3(1,0,0), M_PI/8 ) ;

		rbs.emplace_back( ls, 1. );
		rbs.back().set_velocity( Vec3(0,0,1.e-1), Vec3(0,0,0) ) ;
	}
};
...
struct ImpactScenar : public Scenario {
	...
	{
		LevelSet::Ptr ls = LevelSet::make_sphere() ;
		ls->scale( radius() ).set_origin( .5 * m_config->box + Vec3(0,0,.25*m_config->box[2]) ) ;

		rbs.emplace_back( ls, volMass );
		rbs.back().set_velocity( Vec3(0,0,-zvel), Vec3(avel,0,0) ) ;
	}
...
struct WheelScenar : public Scenario {
	...
	void add_rigid_bodies( std::vector< RigidBody >& rbs ) const override
	{
		...
		ls->scale( radius() )
				.rotate(Vec3(1,0,0), (1-tilt) * M_PI_2)
				.set_origin( .5 * m_config->box + Vec3(.25*m_config->box[0],-.25*m_config->box[1], radius()*(1+2*r)) ) ;

		rbs.emplace_back( ls, volMass );
		rbs.back().set_velocity( Vec3(0,0,-xvel), Vec3(0,-avel,0) ) ;
	}
	...
	void update( Simulate& simu, Scalar /*time*/, Scalar dt ) const override
	{
		...
		for( RigidBody& rb: simu.rigidBodies() ) {
			forces.tail<3>() = rb.levelSet().rotation()
					* Vec3(0,0,torque - angFric * rb.angularVelocity().norm() ) ;
			rb.integrate_gravity( dt, m_config->gravity );
...
struct BunnyScenar : public Scenario {
	...
	void init( const Params& params ) override {
		...
		bunny_ls->scale( S*4 )
				.rotate( Vec3(0,1,0), M_PI/4 )
				.rotate( Vec3(1,0,0), M_PI/4 )
				.set_origin( S * Vec3(.5,.25,-.5) ) ;
		bunny_ls->compute( ) ;
	}
	...
	void update( Simulate& simu, Scalar time, Scalar /*dt*/ ) const override
	{
		...
		if( tw > .5 ) {
			const Scalar t = tw-.5 ;
			if( t < 1 ) {
				vel = Vec3(0,0,6*(t - t*t)) * speed * ( .3 * m_config->box[0] );
			}
		}

		for( RigidBody& rb: simu.rigidBodies() ) {
			rb.set_velocity( vel, Vec3::Zero() );
		}
	}
...
struct WritingScenar : public Scenario {
	...
	void add_rigid_bodies( std::vector< RigidBody >& rbs ) const override
	{
		...
		cyl_ls->scale(R).set_origin(Vec3::Zero()) ;
...
struct DiggingScenar : public Scenario {
	...
	void init( const Params& params ) override {
		...
		left_hand_ls->scale( .5*S )
				.rotate( Vec3(0,1,0), M_PI_2 )
				.set_origin( Vec3(S/2,0,H) ) ;
		left_hand_ls->compute( ) ;

		right_hand_ls->scale( .5*S )
				.rotate( Vec3(0,1,0), -M_PI_2 )
				.rotate( Vec3(0,0,1), -M_PI )
				.set_origin( Vec3(S/2,S,H) ) ;
	...
	void update( Simulate& simu, Scalar time, Scalar /*dt*/ ) const override
	{
		...
		if( tw > 2 ) {
			const Scalar t = tw - 2;
			if( t < 1 ) {
				avel = Vec3(-M_PI_2,0,0);
			}
		}else if( tw > 1 ) {
			const Scalar t = tw - 1;
			if( t < 1 ) {
				vel = Vec3(0, 0, 6*(t-t*t) ) * .4 * m_config->box[2] ;
			}
		}else if( tw > 0 ) {
			const Scalar t = tw;

			const Eigen::AngleAxis<Scalar> firstRot(
					Eigen::AngleAxis<Scalar>(   -M_PI/6, Vec3(0,1,0) ) *
					Eigen::AngleAxis<Scalar>( .8*M_PI_2, Vec3(1,0,0) ) );

			if( t < 1 ) {
				avel = firstRot.angle() * firstRot.axis();
				vel = Vec3(0, std::sin(t) * .75 * m_config->box[1], -std::cos(t) * .7 * m_config->box[2]) ;
			}
		}
		...
		int i = 1 ;
		for( RigidBody& rb: simu.rigidBodies() ) {
			rb.set_velocity( Vec3(vel[0], i*vel[1], vel[2]), Vec3( i*avel[0], avel[1], i*avel[2] ) );
			i = -i ;
		}
	}
