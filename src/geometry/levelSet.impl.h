...
#ifndef MPM_LEVEL_SET_IMPL_H_
#define MPM_LEVEL_SET_IMPL_H_
...
struct PlaneLevelSet : public LevelSet
{
  ...
  Vec3 grad_local(const Vec3 & ) const override {
    return Vec3(0, 0, -1) ;
  }
...
private:
  ...
  static void proj_on_circle( const Vec3&x, Vec3& proj )
  {
    const Scalar n = x.head<2>().norm() ;

    // Projection on unit circle on XY axis
    if( n < 1.e-12 ) proj = Vec3(0,1,0) ;
    else {
...
private:
  ...
  void proj_on_plane( const Vec3&x, Vec3& proj ) const
  {
...
private:
  ...
  void proj_on_axis( const Vec3&x, Vec3& proj ) const
  {
    proj[0] = 0. ;
    proj[1] = 0. ;
...
private:
  ...
  void proj_on_ends( const Vec3&x, Vec3& proj ) const
  {
    proj[0] = 0. ;
    proj[1] = 0. ;
    if(x[2]>0)
